...
# Ergonomic violation thresholds
BACK_UNSAFE  = 40.0
BACK_MOD     = 20.0
KNEE_UNSAFE  = 100.0
KNEE_MOD     = 140.0
...
def back_angle_vertical(shoulder: Tuple[float, float], hip: Tuple[float, float]) -> float:
    return _angle_from_vectors(_vec2(shoulder, hip), (0.0, 1.0))
...
def build_feature_vector(features: Dict[str, float]) -> np.ndarray:
    """Assemble the canonical (1, 15) feature vector for classifier input."""
